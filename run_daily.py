...
# =========================================================
# DYNAMIC PATH CONFIGURATION (Project-Relative)
# =========================================================
...
if not valid_dates:
    print("CRITICAL: No valid historical dates found in catalog. Exiting.")
    sys.exit(1)
...
try:
    ...
    if not hist.empty:
        ...
        if not after.empty:
            ...
            print(f"[Data] yfinance fallback spot price: {spot_price:.2f}")
...
EARNINGS_JSON = os.path.join(CONFIG_DIR, "earnings.json")

event_calendar = EventCalendar(MACRO_JSON, EARNINGS_JSON)
is_macro_clear, macro_reasons = event_calendar.check_trade_date(str(TRADE_DATE), "TSLA")

if not is_macro_clear:
    print(f"\n[!] EVENT WARNING FOR {TRADE_DATE}:")
...
try:
    ...
    print(f"WARNING: FusionPredictor failed ({e}). Using neutral fallbacks.")
...
for idx, candidate in enumerate(candidates, start=1):
    ...
    try:
        ...
        if mu_arith < 0.0 and opt_type == "call":
            print(f"  [Candidate {idx}] Skipped Call due to bearish AI signal.")
            continue
        if mu_arith > 0.0 and opt_type == "put":
            print(f"  [Candidate {idx}] Skipped Put due to bullish AI signal.")
            continue
        ...
        contract_id = f"TSLA{exp_yymmdd}{cp_flag}{strike_val:.1f}"
        ...
        final_decision = decision_builder.build_decision(
            ...
            no_macro_events=is_macro_clear,
            system_healthy=True
        )
        ...
        base_info = {
            ...
            "S0": candidate.get("underlying_price", 0.0),
            "IV0": leg.get("iv", 0.0),
            ...
            "ev_dollars": final_decision.metrics_snapshot.get("ev_dollars", agg.get("expected_pnl_mean", 0.0) * 100),
            "spread_cost_dollars": final_decision.metrics_snapshot.get("spread_cost_dollars", spread_abs * 100)
        }
